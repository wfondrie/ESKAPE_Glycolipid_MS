...
# Import Spectra ---------------------------------------------------------------
path <- c("data/fullLib/Acinetobacter baumannii - res",
          "data/fullLib/Acinetobacter baumannii - sen")
...
ooi <- c("Acinetobacter baumannii - res",
         "Acinetobacter baumannii - sen")
...
dot %>%
  ...
  spread(lib, dotP) %>%
  select(org, starts_with("Acinetobacter baumannii")) %>%
  mutate(`Colistin\nResistance` = ifelse(str_detect(org, "- res"), "+", "-")) %>%
  ggplot(aes(x = `Acinetobacter baumannii - sen`, 
             y = `Acinetobacter baumannii - res`, 
             color = `Colistin\nResistance`)) +
...
ggsave("results/scatterAb.pdf",  width = 70, height = 70, unit = "mm", useDingbats = F)
...
summedPeaks <- peakDat %>%
  ...
  mutate(relInt = relInt / max(relInt),
         ...
         lab = ifelse(relInt == max(relInt) &
                        relInt > 0, "1910", ""),
         lab = ifelse(mz == mz[relInt == max(relInt[mz > 2032 & mz < 2036])] &
                        relInt > 0, "2033", lab),
         lab = ifelse(mz == mz[relInt == max(relInt[mz > 1727 & mz < 1731])] &
                        relInt > 0, "1728", lab),
         lab = ifelse(mz == mz[relInt == max(relInt[mz > 1375 & mz < 1379])] &
                        relInt > 0, "1376", lab),
         lab = ifelse(mz == mz[relInt == max(relInt[mz > 1403 & mz < 1407])] &
                        relInt > 0, "1404", lab),
         xoffset = ifelse(lab %in% c("1910", "1404", "1728", "2033"), xoff, 0),
         xoffset = ifelse(lab %in% c("1376"), -xoff, xoffset))
...
write_csv(labeledPeaks, "results/labeledPeaksAb.csv")
...
ggsave("results/mirrorSpecAb.pdf", width = 140, height = 70, unit = "mm", useDingbats = F)
